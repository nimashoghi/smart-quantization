...
inline quantized_array_t compress(array_t array)
{
    ...
    for (auto i = 0u; i < array.size(); ++i) {
        ...
        return_array[i] = quantize(z_score);
    }

    return return_array;
}

inline array_t decompress(quantized_array_t array, const float mean, const float std_dev)
{
    array_t return_array = {0};

    const auto sum = 0;
    for (auto i = 0u; i < array.size(); ++i) {
        const auto z_score = dequantize(array[i]);
...
extern "C"
{
    void compress_accel(axi_interface_type *src, axi_interface_type *dst)
    {
...
#pragma UNROLL
    ...
    void decompress_accel(const float mean, const float std_dev, axi_interface_type *src, axi_interface_type *dst)
    {
